...
import dxe.echonet.property.group01.AirCleanerA0;
import dxe.echonet.property.group01.AirCleanerC0;
import dxe.echonet.property.group01.AirCleanerC1;
import dxe.echonet.property.group01.AirCleanerC2;
import dxe.echonet.property.group01.AirCleanerE1;
...
public class AirCleaner extends ECHONETObject {
	@JsonIgnore
	private static final Logger logger = Logger.getLogger(AirCleaner.class.getName());

	public AirCleaner(Node node, EOJ eoj, Service s) {
		super(node, eoj, s);
		String topic = String.format("%s/register", Controller.publishTopic);
		String payload = new JSONObject().put("deviceType", "airCleaner").put("id", getDeviceID()).toString();
		...
		try {
			this.service.doGet(this.getNode(), this.getEoj(), this.getGettableEPCList(), 5000, new GetListener() {
				...
				public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					case xA0:
						c += 1;
						ELProperty ppa0 = new AirCleanerA0(resultData.toBytes());
						getProperties().put(EPC.xA0, ppa0);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB0, EDT: 0x%02X}=={airFlowLevel:%s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
								ppa0.edtToStringValue()));
						break;
					case xC0:
						c += 1;
						ELProperty ppc0 = new AirCleanerC0(resultData.toBytes());
						getProperties().put(EPC.xC0, ppc0);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB1, EDT: 0x%02X}=={airPollutionDetection: %s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
								ppc0.edtToStringValue()));
						break;
					case xC1:
						c += 1;
						ELProperty ppc1 = new AirCleanerC1(resultData.toBytes());
						getProperties().put(EPC.xC1, ppc1);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB1, EDT: 0x%02X}=={smokeDetection: %s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
								ppc1.edtToStringValue()));
						break;
					case xC2:
						c += 1;
						ELProperty ppc2 = new AirCleanerC2(resultData.toBytes());
						getProperties().put(EPC.xC2, ppc2);
						logger.info(String.format(
								"Node:%s@EOJ:%s {EPC:0xB1, EDT: 0x%02X}=={opticalCatalystOperationStatus: %s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
								ppc2.edtToStringValue()));
						break;
					case xE1:
						c += 1;
						ELProperty ppe1 = new AirCleanerE1(resultData.toBytes());
						getProperties().put(EPC.xE1, ppe1);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB1, EDT: 0x%02X}=={filterChangeNotice: %s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
								ppe1.edtToStringValue()));
						break;
