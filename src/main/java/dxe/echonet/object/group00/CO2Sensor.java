...
import dxe.echonet.property.group00.Co2SensorE0;
...
public class CO2Sensor extends ECHONETObject {
	@JsonIgnore
	private static final Logger logger = Logger.getLogger(CO2Sensor.class.getName());

	public CO2Sensor(Node node, EOJ eoj, Service s) {
		super(node, eoj, s);
		String topic = String.format("%s/register", Controller.publishTopic);
		String payload = new JSONObject().put("deviceType", "co2Sensor").put("id", getDeviceID()).toString();
		...
		try {
			this.service.doGet(this.getNode(), this.getEoj(), this.getGettableEPCList(), 5000, new GetListener() {
				...
				public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					case xE0:
						c += 1;
						ELProperty ppE0 = new Co2SensorE0(resultData.toBytes());
						getProperties().put(EPC.xE0, ppE0);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB0, EDT: 0x%02X}=={Co2:%s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
