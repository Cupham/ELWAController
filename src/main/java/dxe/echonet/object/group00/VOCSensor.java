...
import dxe.echonet.property.group00.VOCSensorB0;
import dxe.echonet.property.group00.VOCSensorB1;
import dxe.echonet.property.group00.VOCSensorE0;
...
public class VOCSensor extends ECHONETObject {
	@JsonIgnore
	private static final Logger logger = Logger.getLogger(VOCSensor.class.getName());

	public VOCSensor(Node node, EOJ eoj, Service s) {
		super(node, eoj, s);
		String topic = String.format("%s/register", Controller.publishTopic);
		String payload = new JSONObject().put("deviceType", "vocSensor").put("id", getDeviceID()).toString();
		...
		try {
			this.service.doGet(this.getNode(), this.getEoj(), this.getGettableEPCList(), 5000, new GetListener() {
				...
				public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					case xB0:
						c += 1;
						ELProperty ppb0 = new VOCSensorB0(resultData.toBytes());
						getProperties().put(EPC.xB0, ppb0);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB0, EDT: 0x%02X}=={thresholdLevel:%s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
								ppb0.edtToStringValue()));
						break;
					case xB1:
						c += 1;
						ELProperty ppb1 = new VOCSensorB1(resultData.toBytes());
					...
					case xE0:
						c += 1;
						ELProperty ppe0 = new VOCSensorE0(resultData.toBytes());
						getProperties().put(EPC.xE0, ppe0);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB2, EDT: 0x%02X}=={Value:%s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
