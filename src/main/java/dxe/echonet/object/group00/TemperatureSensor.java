...
import dxe.echonet.property.group00.TemperatureSensorE0;
...
public class TemperatureSensor extends ECHONETObject {
	@JsonIgnore
	private static final Logger logger = Logger.getLogger(TemperatureSensor.class.getName());

	public TemperatureSensor(Node node, EOJ eoj, Service s) {
		super(node, eoj, s);
		String topic = String.format("%s/register", Controller.publishTopic);
		String payload = new JSONObject().put("deviceType", "temperatureSensor").put("id", getDeviceID()).toString();
		...
		try {
			this.service.doGet(this.getNode(), this.getEoj(), this.getGettableEPCList(), 5000, new GetListener() {
				...
				public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					case xE0:
						c += 1;
						ELProperty ppe0 = new TemperatureSensorE0(resultData.toBytes());
						getProperties().put(EPC.xE0, ppe0);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB0, EDT: 0x%02X}=={Temperature:%s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
