...
import dxe.echonet.property.group00.IlluminanceSensorE0;
import dxe.echonet.property.group00.IlluminanceSensorE1;
...
public class IlluminanceSensor extends ECHONETObject {
	@JsonIgnore
	private static final Logger logger = Logger.getLogger(IlluminanceSensor.class.getName());

	public IlluminanceSensor(Node node, EOJ eoj, Service s) {
		super(node, eoj, s);
		String topic = String.format("%s/register", Controller.publishTopic);
		String payload = new JSONObject().put("deviceType", "illuminanceSensor").put("id", getDeviceID()).toString();
		...
		try {
			this.service.doGet(this.getNode(), this.getEoj(), this.getGettableEPCList(), 5000, new GetListener() {
				...
				public void receive(GetResult result, ResultFrame resultFrame, ResultData resultData) {
					...
					case xE0:
						c += 1;
						ELProperty ppe0 = new IlluminanceSensorE0(resultData.toBytes());
						getProperties().put(EPC.xE0, ppe0);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB0, EDT: 0x%02X}=={Value in Lux:%s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
								ppe0.edtToStringValue()));
						break;
					case xE1:
						c += 1;
						ELProperty ppe1 = new IlluminanceSensorE1(resultData.toBytes());
						getProperties().put(EPC.xE1, ppe1);
						logger.info(String.format("Node:%s@EOJ:%s {EPC:0xB1, EDT: 0x%02X}=={Value in KLux: %s}",
								getNode().getNodeInfo().toString(), getEoj().toString(), resultData.toBytes()[0],
					...
					System.out.println(c + "===========================" + getGettableEPCList().size());
