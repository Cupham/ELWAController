...
public class ElectricCurtain extends ECHONETObject {
	@JsonIgnore
	private static final Logger logger = Logger.getLogger(ElectricCurtain.class.getName());

	public ElectricCurtain(Node node, EOJ eoj, Service s) {
		super(node, eoj, s);
		String topic = String.format("%s/register", Controller.publishTopic);
		String payload = new JSONObject().put("deviceType", "electricCurtain").put("id", getDeviceID()).toString();
